...
function AwardImage({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    <div className="aspect-[210/297] bg-stone-100 dark:bg-stone-900 relative rounded-xl border border-stone-200 dark:border-stone-800 overflow-hidden">
      {failed ? (
      ...
      <div>
        {recognition.items.map((item, i) => (
          <Reveal key={item.title}>
            <article
              className={`grid md:grid-cols-[minmax(0,380px)_1fr] gap-8 md:gap-12 items-center py-10 ${
                i > 0 ? "border-t border-stone-200 dark:border-stone-800" : ""
              }`}
            >
